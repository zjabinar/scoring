      ...
      for (var iCol = 0; iCol < this.grid.fields.length; iCol++) {
        if (objArgs.cells[iCol] &&
         typeof objArgs.cells[iCol].data != 'undefined') {
          // Update cell
          this.grid.rows[iRow].cells[iCol] = this.grid.fields[iCol].newCell({
            row: this.grid.rows[iRow],
            value: objArgs.cells[iCol].data,
            userStyle: objArgs.cells[iCol].style
          });
        }
      }
