...
<!--
function message_cri(criteria) {
    alert("Criteria is now Activated");
}
// -->
</script>
<?php
...
if (isset($_REQUEST['criteria_id'])) {
	if($_REQUEST['criteria_id']<>0 or $_REQUEST['criteria_id']<>"") {
		...
		$query_set_cri = "SELECT criteria_id, criteria_name FROM final_criteria where criteria_id=".$_REQUEST['criteria_id'];
...
$query_rec_semi = "SELECT criteria_id, criteria_name FROM final_criteria WHERE  {$NXTFilter_rec_semi}  ORDER BY  {$NXTSort_rec_semi} ";
          ...
          <?php if ($totalRows_rec_semi > 0) { // Show if recordset not empty ?>
            <?php do { ?>
              <tr class="<?php echo @$cnt1++%2==0 ? "" : "KT_even"; ?>">
                ...
                <td><a class="KT_edit_link" href="set_criteria_sub_final.php?criteria_id=<?php echo $row_rec_semi['criteria_id']; ?>">Activate</a></td>
              </tr>
...
mysql_free_result($set_cri);

mysql_free_result($set_criteria);
?>
