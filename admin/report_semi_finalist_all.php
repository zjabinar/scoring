...
mysql_select_db($database_conn_mutya, $conn_mutya);
$query_contestants = "SELECT contestant_id, contestant_num, firstname, lastname, middlename, birthdate, municipality FROM contestants";
...
do { 
	...
	mysql_select_db($database_conn_mutya, $conn_mutya);
	$query_pre_scores = "SELECT pre_scores.contestant_id, Sum(pre_scores.rating) AS total_score FROM pre_scores GROUP BY pre_scores.contestant_id HAVING pre_scores.contestant_id=". $contestant_id;
	...
	$scores = ($row_pre_scores['total_scores'] + $row_semi_scores['total_scores'])/ $count_criteria;
